...
void record_preferences(int ranks[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        for (int j = i + 1; j < candidate_count; j++)
        {
            ...
            // Preferences[i][j] represents the number of people who prefer candidate i over candidate j
...
void add_pairs(void)
{
    // Iterate over the preferences 2D matrix
    // Upper triangular
    for (int row = 0; row < MAX; row++)
    {
        for (int col = row + 1; col < MAX; col++)
        {
            // Comparing how many voters prefer the i candidate over the j condidate, and vice versa
...
void sort_pairs(void)
{
    for (int i = 0; i < pair_count; i++)
    {
        ...
        // Find the pair with the max strength of victory
...
// Print the winner of the election - by finding the source of the graph
void print_winner(void)
{
